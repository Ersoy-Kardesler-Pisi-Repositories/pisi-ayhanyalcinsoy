...
class ComponentDBTestCase(testcase.TestCase):
    ...
    def testListComponents(self):
        assert set(self.componentdb.list_components("pardus-2007")) == set(
            ["system", "system.base", "applications", "applications.network"]
        )
        assert set(self.componentdb.list_components("contrib-2007")) == set(
            ["applications", "applications.util", "applications.network"]
        )
        assert set(self.componentdb.list_components()) == set(
            [
                "system",
                "system.base",
                "applications",
    ...
    def testGetComponent(self):
        ...
        component = self.componentdb.get_component(
            "applications.network", "contrib-2007"
        )
        assert component.name == "applications.network"
    ...
    def testGetPackages(self):
        ...
        packages = self.componentdb.get_packages(
            "applications", "contrib-2007", walk=True
        )
    ...
    def testSearchComponent(self):
        ...
        assert set(packages) == set(
            ["applications", "applications.network", "applications.util"]
        )

        packages = self.componentdb.search_component(
            ["system", "base"], repo="pardus-2007"
        )
        assert set(packages) == set(["system.base"])

        packages = self.componentdb.search_component(
            ["system", "base"], repo="contrib-2007"
        )
        assert not packages
