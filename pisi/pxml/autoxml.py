...
class autoxml(type):
    ...
    def __init__(cls, name, bases, dct):
        ...
        def errors(self, where=str(cls.tag)):
            errs = []
            # Check if this class has an explicit errors method (not generated by autoxml)
            if hasattr(cls, '_has_explicit_errors') and cls._has_explicit_errors:
                # Use the explicit errors method - call the original method
                return super(type(self), self).errors(where)
            for base in cls.autoxml_bases:
                errs.extend(base.errors(where))
            for error_func in errorss:
                errs.extend(error_func(self, where))
