...
class Builder:
    ...
    def fetch_translationsfile(self):
        translationsuri = util.join_path(self.specdiruri, ctx.const.translations_file)
        try:
    ...
    def fetch_patches(self):
        for patch in self.spec.source.patches:
            ...
            patchuri = util.join_path(self.specdiruri, ctx.const.files_dir, patch.filename)
            self.download(patchuri, util.join_path(self.destdir, ctx.const.files_dir, dir_name))

    def fetch_comarfiles(self):
        for package in self.spec.packages:
            for pcomar in package.providesComar:
                comaruri = util.join_path(self.specdiruri, ctx.const.comar_dir, pcomar.script)
                self.download(comaruri, util.join_path(self.destdir, ctx.const.comar_dir))
    ...
    def get_abandoned_files(self):
        ...
        def is_included(path1, path2):
            return path1 == path2 or fnmatch.fnmatch(path1, path2) or (not os.path.isfile(path2) and fnmatch.fnmatch(path1, util.join_path(path2, "*")))
        ...
        return list(map(lambda x: x[len_install_dir:], abandoned_files))
        ...
        if not os.path.exists(src_dir):
            ...
            while not os.path.exists(src_dir):
                ...
                if not ext:
                    break
            if not os.path.exists(src_dir):
                src_dir = util.join_path(self.pkg_work_dir(), 
                                         [d for d in next(os.walk(self.pkg_work_dir()))[1] if not d.startswith(".")][0])
        ...
        for package in self.spec.packages:
            for path_info in package.files:
                ...
                if not path.startswith("/"):
                    raise Error(_("Source package '%s' defines a relative 'Path' element: %s") % (self.spec.source.name, path_info.path))

                if path in paths:
                    raise Error(_("Source package '%s' defines multiple 'Path' tags for %s") % (self.spec.source.name, path_info.path))
...
def check_build_dependencies(self):
    ...
    if not ctx.config.values.general.ignore_safety and \
        ...
        if self.componentdb.has_component('system.devel'):
            build_deps_names = {x.package for x in build_deps}
            ...
            extra_names = list(extra_names)  # Convert filter object to list for Python 3 compatibility
            if extra_names:
    ...
    def add_path(path):
        for fpath, fhash in util.get_file_hashes(path, collisions, install_dir):
            if ctx.get_option('create_static') \
                ...
                continue
            frpath = util.removepathprefix(install_dir, fpath)
            ftype, permanent = get_file_type(frpath, package.files)
            fsize = int(util.dir_size(fpath))
            st = os.lstat(fpath) if os.path.islink(fpath) else os.stat(fpath)
