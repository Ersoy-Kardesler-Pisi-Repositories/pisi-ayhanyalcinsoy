...
class PGraph(graph.Digraph):
    ...
    def __init__(self, packagedb):
        super().__init__()  # Using Python 3 style super
    ...
    def add_dep(self, pkg, depinfo):
        ...
        if depinfo.package not in self.vertices():
            ...
            pkg2data = (pkg2.version, pkg2.release)
        self.add_edge(str(pkg), str(depinfo.package), ('d', depinfo), pkg1data, pkg2data)

    def write_graphviz_vlabel(self, f, u):
        (v, r) = self.vertex_data(u)
        f.write(f'[ label = "{u}({v},{r})" ]')  # Using f-string for formatting
