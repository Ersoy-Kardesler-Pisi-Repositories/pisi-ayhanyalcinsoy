...
import os
import re
import io  # StringIO için io modülü
import configparser  # ConfigParser için configparser
...
class ConfigurationSection:
    ...
    def __init__(self, section, items=[]):
        ...
        if section == "general":
            ...
            raise Error(e)  # Python 3'te raise Error, e yerine raise Error(e)
...
class ConfigurationFile:
    ...
    def __init__(self, filePath):
        self.parser = configparser.ConfigParser()  # ConfigParser güncellendi
    ...
    def write_config(self, add_missing=True):
        sections = {}
        current = io.StringIO()  # StringIO güncellendi
        replacement = [current]
        sect = None
        opt = None
        written = []
        ...
        with open(self.filePath, "r+") as fp:  # open() içinde with kullanıldı
            padded_vi = " = "
            while True:
                line = fp.readline()
                if not line:
            ...
            for sect in sects:
                ...
                for opt in opts:
                    if opt != "__name__" and not (sect, opt) in written:
                        ...
                        output.write("%s%s%s\n" % (opt, padded_vi, value))
                        written.append((sect, opt))
        with open(self.filePath, "w") as fp:  # Yeniden yazmak için aç
